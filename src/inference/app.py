...
from .schemas import (
    ...
    BatchPredictionResponse, ModelInfo, HealthCheckResponse,
    PatientData, FeatureContribution
)
...
async def health_check():
    ...
    """
    is_loaded = model_handler.is_model_loaded()
    model_version = None
...
async def predict(request: PredictionRequest):
    ...
    try:
        ...
        # Make prediction
        result = model_handler.predict(patient_data)
...
async def batch_predict(request: BatchPredictionRequest):
    ...
    try:
        ...
        # Make predictions
        results = model_handler.batch_predict(patients_data)
        ...
        response = BatchPredictionResponse(
            ...
            model_version=model_handler.get_model_info().get('version', 'unknown'),
...
def start():
    ...
    # Start server
    uvicorn.run("src.inference.app:app", host="0.0.0.0", port=port, reload=True)
